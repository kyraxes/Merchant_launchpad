...
export function PosterStudio({ merchant, initialLocale }: { merchant: Merchant; initialLocale: Locale }) {
  ...
  useEffect(() => {
    ...
    async function draw() {
      ...
      ctx.globalAlpha = 1;
      const pad = 76*scale; ctx.fillStyle = palette.ink; ctx.font = `700 ${28*scale}px Arial`; ctx.fillText(merchant.name[locale].toUpperCase(), pad, pad);
      ctx.font = `900 ${190*scale}px Arial`; ctx.fillText(merchant.emoji, pad, pad+235*scale);
      ...
      ctx.font = `500 ${24*scale}px Arial`; ctx.fillText(merchant.area[locale], pad+36*scale, cardY+112*scale); ctx.fillText(merchant.phone, pad+36*scale, cardY+153*scale);
      ctx.font = `600 ${18*scale}px Arial`; ctx.fillText("MOCK V1 · Merchant Launchpad", pad+36*scale, cardY+cardH-35*scale);
      if (qrTarget !== "none") {
        const origin = window.location.origin;
        const targets = { website: `${origin}/${locale}/stores/${merchant.slug}`, google: `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(merchant.name.en)}`, line: `https://line.me/R/ti/p/${merchant.lineId}` };
  ...
  }, [merchant, product, locale, format, theme, qrTarget]);
