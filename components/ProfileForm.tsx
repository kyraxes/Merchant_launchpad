...
export function ProfileForm({ locale }: { locale: Locale }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setBusy(true);
    try {
      await updateMerchant(form, locale, idToken);
  ...
  return <form className="profile-form" onSubmit={submit}>
    ...
    <button className="save-button" type="submit" disabled={busy}>{busy ? t.submitting : t.submit}</button>
    {draft.status !== "published" && <button className="reset-button danger-text" type="button" disabled={busy} onClick={() => void remove()}>{t.delete}</button>}
  </form>;
}
